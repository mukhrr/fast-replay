...
export interface StepDefinition {
  ...
  /**
   * Present on steps generated by `repro extract`: the IR steps its `run`
   * replays. Lets `repro extract` recognize a later recording that re-drove
   * this preamble by hand. Untyped here so step files stay loadable without
   * the IR module; consumers validate it against the step schema.
   */
  fragment?: unknown[];
...
/**
 * Every step reachable through `requires` from `names`, the names included.
 *
 * Cycle-safe. Unknown names pass through untouched so `runStep` keeps owning
...
export function transitiveRequires(
  ...
  const visit = (name: string): void => {
    ...
    seen.add(name);
    for (const dependency of steps.get(name)?.requires ?? []) visit(dependency);
  };
