...
**IR** (`src/ir/`): the zod schema in `schema.ts` is the source of truth for the on-disk JSON (`.repros/<name>/`), with `IR_VERSION` bumped on non-backward-readable changes. `edit.ts` backs `repro fix` / `repro assert`. Key concepts: `target.identity` (the text that must match before acting — replay verifies it and refuses rather than acts on the wrong element), `assertion.mode` (`expect-bug` vs `expect-fixed`), `observedAtRecord` (the bug's signature: console errors and failed requests, minus per-repro ambient noise via `src/noise.ts` and minus anything captured while a setup step ran, the same window replay judges), and `expectedWhenFixed` (hand-written fix criterion; without it `--expect-fixed` refuses rather than passing a check that checked nothing).

**Replayer** (`src/replayer/`): `run.ts` drives the browser from IR. Failure `kind` distinguishes `assertion` (a verdict on the bug) from `infrastructure` (`COULD NOT VERIFY` — the harness couldn't drive the app, which says nothing about the bug). A recorded reaction that never arrives before the last step is `infrastructure` too, because the flow never reached the bug. `resolve.ts` takes the first rendered match a user can reach, ahead of a copy under `aria-hidden` or `inert` (a background screen kept mounted), and falls back to the first rendered match when every match is under one. `retarget.ts` implements `--env` (moves goto steps, same-origin network patterns and the session onto a new origin). `values.ts` expands `{{random:*}}` placeholders. Failure artifacts (screenshot, console tail, network log) land in `.repros/<name>/artifacts/`.
...
**Jev** (`src/jev/`): optional, record time only. `key.ts` resolves `TYPESAFE_API_KEY` or `~/.config/fast-replay/credentials.json`; `client.ts` calls the TypeSafe API over `fetch`; `page.ts` reads candidates as element handles (never writing to the DOM the recorder watches), keeping only controls a click can reach and naming the row and dialog or region each sits in, and waits for that list to stop changing; it also reads the page state with passwords masked; `driver.ts` turns a goal into an ordinary `drive` function, waiting for the requests an action started (quiet 500 ms, capped at 10 s) before checking `--until`. A goal that does not reach `--until` writes nothing, including when the run is stopped early. `src/notice.ts` prints the once-per-version first-run notice.
